...
shinyServer(function(input, output) {
  ...
  output$B_MSE<-renderTable({
    ...
    if(names(datc)[1]== "case"){
      ...
      error<-sum(abs(predicy-y))/length(y)
  ...
  output$poste<-renderPlot({
    ...
    if(names(datc)[1]== "case"){
      ...
      plot(Iteration, predicy, xlab="Case Number", ylab="Class of status 'y' " ,ylim =c(-0.1, 1.1), col="red")
      
      points(y-0.05,pch=16, col="blue")
      
      legend("right",legend=c("Posterior","Original 'y' "),pch=c(1,16),col=c("red","blue"))
      ...
      #legend("right",legend=c("Posterior Mean","Actual"),pch=c(1,16),col=c("red","blue"))
      
      #mupred<-mupred[order(mupred)]
      plot(y, mupred,xlab="Actual ", ylab="Posterior Mean" ,lwd= 1) 
